...
*/
    ...
    /*
    size_t find (char c, size_t pos = 0);ищет первое появление символа c в строке, от имени которой он вызывается:
    Поиск начинается с позиции (индекса) pos. Если pos не задано, то поиск начинается с начала строки. 
    Возвращается индекс позиции этого символа в строке, либо значение string::npos

    string substr (size_t pos = 0, size_t len = npos); позволяет получить подстроку от той строки, от имени которой он вызывается. 
    Подстрока начинается с индекса pos и имеет длину len символов:
    Если количество символов len не указано, то по умолчанию берется подстрока от позиции pos до конца исходной строки. 
    Метод возвращает новую строку типа string

    string& insert (size_t pos, size_t n, char c); вставляет символы в строку  начиная с позиции pos. Вставляемый символ  повторяется n раз.      
    string& insert (size_t pos, const string& str); Аналогично в исходную строку вставляется подстрока str начиная с позиции pos
    Метод возвращает ссылку на исходную строку. Его результат можно проигнорировать:

    string& erase (size_t pos = 0, size_t len = npos); удаляет часть строки, уменьшая ее длину. 
    Удаляемый фрагмент начинается с позиции pos и состоит из len символов. 
    Если второй параметр не указан - то  удаляются все символы до конца строки.
    Метод возвращает ссылку на исходную строку. Его результат также можно проигнорировать:
