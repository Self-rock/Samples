...
/* Этот код удаляет элементы из контейнера v, которые удовлетворяют определённому условию.
std::remove_if — это алгоритм, который перемещает все элементы контейнера, не удовлетворяющие заданному условию, в конец контейнера.
Первый аргумент std::remove_if — это диапазон, из которого нужно удалить элементы. В данном случае это v.begin() и v.end().
Второй аргумент — это условие, которому должны удовлетворять элементы. В этом случае проверяется, равен ли элемент 4.
Результат работы std::remove_if сохраняется в контейнер и возвращает итератор, 
указывающий на позицию, после которой находятся все элементы, подлежащие удалению.
Затем используется метод erase, который удаляет элементы из исходного контейнера, начиная с позиции, 
возвращённой std::remove_if, и до конца контейнера (второй аргумент erase — v.end()). */
    
    auto v12 = std::vector{3, 1, 4};
    v12.erase(
       std::remove_if(v12.begin(), v12.end(), [](auto&& elt) {
           return elt == 4;
       }),
       v12.end()
   );
